# 🚂 Copilot Instructions for MOBAflow Project

**Quick Reference Guide** - See linked documents for detailed guidelines.

---

## 🌐 Communication Language

**Chat Language**: **Deutsch (German)**
- ✅ All chat conversations with the user should be conducted in **German**
- ✅ Code, documentation, and comments remain in **English**

---

## ✅ Execution Discipline (MANDATORY)
- After each prompt execution: run build, run all tests, and reduce warnings when possible.
- Consider changes complete only after a green build/test.

---

## 📋 Project Overview

**MOBAflow** - Model railroad control system built with .NET 10
- **Primary Platform**: Windows (WinUI 3) - "MOBAflow"
- **Secondary Platform**: Android (MAUI) - "MOBAsmart"  
- **Web Platform**: Blazor Server - "MOBAdash"
- **Target Hardware**: Z21 digital command station (UDP)

---

## 🏗️ Architecture (MANDATORY!)

### **Core Principle: Keep Backend Platform-Independent**

⚠️ **CRITICAL RULE**: The `Backend` project MUST remain **100% platform-independent**!

- ❌ **NEVER** add UI thread dispatching to Backend (`DispatcherQueue`, `MainThread`, etc.)
- ✅ **ALWAYS** handle platform-specific threading in **platform-specific ViewModels**

```csharp
// ❌ WRONG: Dispatching in Backend
public class JourneyManager
{
#if WINDOWS
    await DispatchToUIThreadAsync(...);  // ❌ BAD!
#endif
}

// ✅ CORRECT: Platform-specific ViewModel handles dispatching
namespace Moba.WinUI.ViewModels.Journey;

public class JourneyViewModel : SharedUI.ViewModel.JourneyViewModel
{
    private readonly DispatcherQueue _dispatcher;
    // Handle UI thread dispatching here ✅
}
```

---

## 🎨 Project Structure

| Project | Purpose | Target Framework |
|---------|---------|------------------|
| **Backend** | Z21 communication, business logic (platform-independent!) | `net10.0` |
| **SharedUI** | Base ViewModels, shared UI logic | `net10.0` |
| **WinUI** | Windows desktop "MOBAflow" | `net10.0-windows10.0.17763.0` |
| **MAUI** | Android mobile "MOBAsmart" | `net10.0-android36.0` |
| **WebApp** | Blazor Server "MOBAdash" | `net10.0` |
| **Sound** | Audio/TTS functionality | `net10.0` |
| **Test** | Unit tests | `net10.0` |

### Dependency Flow
```
WinUI → SharedUI → Backend
MAUI → SharedUI → Backend
WebApp → SharedUI → Backend
```

---

## 📁 File Organization

### ✅ **One Class Per File**
- File name MUST match class name (e.g., `SystemState.cs`)
- ❌ **NEVER** put multiple public classes in one file

### ✅ **Use Sub-Namespaces for Organization**
```csharp
// ✅ GOOD: Organized with sub-namespaces
namespace Moba.WinUI.ViewModels.Journey;
namespace Moba.WinUI.Services;

// ❌ BAD: Flat structure
namespace Moba.WinUI.ViewModel;  // WinUIJourneyViewModel, WinUIMainWindowViewModel, etc.
```

---

## 📁 SharedUI Platform-specific subfolders

To keep platform-specific ViewModels organized while still residing in `SharedUI`, place platform adapters under these subfolders:

- `SharedUI/ViewModel/WinUI` - WinUI-specific ViewModel adapters that dispatch to `DispatcherQueue` and/or use WinUI services.
- `SharedUI/ViewModel/MAUI` - MAUI-specific ViewModel adapters that dispatch to `MainThread` and/or use MAUI services.

These adapters must remain thin: inherit from `SharedUI.ViewModel.*` base ViewModels and perform only UI-thread dispatching or platform service wiring.

Example:

```csharp
namespace Moba.SharedUI.ViewModel.WinUI;

public class JourneyViewModel : Moba.SharedUI.ViewModel.JourneyViewModel
{
    // WinUI dispatching only
}
```

Follow the project's file and namespace conventions when adding these adapters.

---

## 🎯 Mandatory Pre-Flight Checks

**BEFORE implementing ANY feature request:**

1. ✅ **Search Microsoft Learn** for official documentation
2. ✅ **Check for known issues** in .NET 10 / MAUI 10 / WinUI 3
3. ✅ **Propose alternatives** if technology is unstable
4. ✅ **Wait for explicit approval** before coding

### Decision Gate
```
IF (technology is stable AND documented AND appropriate)
    THEN implement
ELSE
    PROPOSE alternatives AND wait for user decision
```

---

## 🛠️ Technology Constraints

### .NET 10 LTS (Final Release)
- ✅ Production-ready (released May 20, 2025)
- ✅ Visual Studio 2026 (18.x) required
- ✅ Use C# 13 features where appropriate

### WinUI 3
- ✅ Windows App SDK
- ⚠️ **No XAML Designer** → Use XAML Hot Reload
- ✅ Use **x:Bind** over **Binding**

### MAUI 10 (Final Release)
- ✅ Production-ready for standard apps
- ✅ Android API 36 stable
- ⚠️ **Foreground Services still unstable** → Use native Android
- ⚠️ **Threading Rules CRITICAL!** → See [docs/THREADING.md](docs/THREADING.md)

---

## 🧵 Threading (CRITICAL for MAUI!)

⚠️ **Only Main Thread can modify UI properties!**

```csharp
// ❌ BAD: Background thread
private void OnUdpCallback(Data data)
{
    StatusText = "Received!";  // ❌ CRASH on Android!
}

// ✅ GOOD: Dispatched to Main Thread
private void OnUdpCallback(Data data)
{
#if ANDROID || IOS || MACCATALYST || WINDOWS
    MainThread.BeginInvokeOnMainThread(() =>
    {
        StatusText = "Received!";  // ✅ Safe
    });
#endif
}
```

📄 **Full Guidelines**: See [docs/THREADING.md](docs/THREADING.md)

---

## 🚀 Async/Await Patterns

### Key Rules:
- ✅ Use `async` for I/O-bound work (network, file, database)
- ✅ Suffix methods with `Async`
- ✅ Use `ConfigureAwait(false)` in Backend (not in ViewModels)
- ❌ **NEVER** use `.Result` or `.Wait()` (causes deadlocks)
- ❌ **NEVER** use `async void` (except event handlers)

📄 **Full Guidelines**: See [docs/ASYNC-PATTERNS.md](docs/ASYNC-PATTERNS.md)

---

## 🎨 MVVM Pattern

- ✅ Use **CommunityToolkit.Mvvm** (`ObservableObject`, `RelayCommand`)
- ✅ **ALL** UI logic in ViewModels (never in code-behind)
- ✅ ViewModels must be **testable** (no direct UI dependencies)

```csharp
public partial class MyViewModel : ObservableObject
{
    [ObservableProperty]
    private string title = "Default";

    [RelayCommand]
    private async Task DoSomethingAsync()
    {
        // Logic here
    }
}
```

---

## 🔌 Z21 Hardware & Protocol

**Quick Reference:**
- 📡 **Protocol**: UDP on port 21105
- 🏠 **Default IP**: 192.168.0.111
- 📦 **Implementation**: `Backend.Z21` class
- ⚠️ **Thread Safety**: All events on background thread → Use `MainThread.BeginInvokeOnMainThread()`
- 📄 **Specification**: [Z21 LAN Protocol V1.13 (PDF)](https://www.z21.eu/media/Kwc_Basic_DownloadTag_Component/47-1652-959-downloadTag/default/69bad87e/1699290251/z21-lan-protokoll.pdf)

### Track Feedback System:
- **InPorts**: Track occupancy detectors
- **Current Setup**: 3 parallel tracks → InPorts 1, 2, 3
- **Use Case**: Lap counting (e.g., cleaning train runs)

---

## 📚 Documentation Sources

### Primary Sources:
1. **Microsoft Learn** - https://learn.microsoft.com/dotnet/
2. **GitHub Issues** - .NET MAUI, WinUI, .NET
3. **NuGet Docs** - CommunityToolkit.Mvvm, CommunityToolkit.Maui, UraniumUI
4. **Z21 Protocol** - See PDF link above

### Search Strategy:
```
1. Search Microsoft Learn first
2. Check GitHub issues for known problems
3. Verify with NuGet package docs
4. Consult Z21 Protocol PDF for hardware details
5. Present findings to user BEFORE coding
```

---

## 🚨 Known Issues

### .NET 10 / MAUI 10 Final:
- ⚠️ **MAUI Foreground Services**: Still unstable → Native Android implementation
- ⚠️ **Shell Navigation**: Can be problematic → Prefer `NavigationPage`
- ⚠️ **Hot Reload with Source Generators**: May not update `ObservableProperty` → Restart debugger

---

## 📖 Detailed Documentation

- 📐 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) - Platform-independent Backend guidelines
- 🧵 [docs/THREADING.md](docs/THREADING.md) - MAUI Threading (CRITICAL!)
- 🚀 [docs/ASYNC-PATTERNS.md](docs/ASYNC-PATTERNS.md) - Async/Await best practices
- 🔌 [docs/DI-INSTRUCTIONS.md](docs/DI-INSTRUCTIONS.md) - Dependency Injection guide

---

## ✅ Quick Checklist

**Before committing code:**

- [ ] Backend has NO platform-specific code (`#if WINDOWS`, `MainThread`, etc.)
- [ ] UI updates dispatched to Main Thread (MAUI) or DispatcherQueue (WinUI)
- [ ] All I/O operations use async/await
- [ ] No `.Result` or `.Wait()` on async code
- [ ] File names match class names
- [ ] XML documentation in English
- [ ] Unit tests can run without UI framework
