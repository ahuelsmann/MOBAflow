# 🚂 Copilot Instructions for MOBAflow Project
...
- ✅ All chat conversations with the user should be conducted in **German**
- ✅ Code, documentation, and comments remain in **English**
...
## 🎯 MVP Goals & Product Vision

📋 **Active Plan**: See [Plans/MVP-CRUD-Editor.md](Plans/MVP-CRUD-Editor.md) for detailed implementation roadmap
...
✅ **Complete Model Editability** - All model classes in the solution tree must be editable:
- ✅ **Solution** (Root element)
  - ✅ **Project**
    - ✅ **Settings** (configuration for Z21, voices, etc.)
    - ✅ **Journey** (complete journey/timetable with all stops)
      - ✅ **Station** (individual stop with platforms and workflow)
        - ✅ **Platform** (track/platform assignment)
    - ✅ **Workflow** (automation workflows)
      - ✅ **Action** (individual actions: Audio, Announcement, Command, etc.)
    - ✅ **Train** (complete train compositions)
      - ✅ **Locomotive** (engine specifications)
      - ✅ **Wagon** (PassengerWagon, GoodsWagon)
...
- ✅ **TreeView Navigation** - Solution → Project → Journeys/Workflows/Trains
- ✅ **PropertyGrid Editing** - Edit all properties for selected entity
- ✅ **Context Menus** - Add/Delete operations via right-click
- ✅ **Drag & Drop** (optional) - Reorder Stations in Journey, Actions in Workflow
- ✅ **Validation** - Prevent invalid operations (e.g., delete Station that's referenced)

#### **Current State (as of architecture analysis):**

- ✅ **Read** - TreeView shows full hierarchy ✅
- ✅ **Update** - PropertyGrid allows editing ✅
- ⚠️ **Create** - Limited support (Project creation exists, but not full CRUD for all entities)
...
⚠️ **CRITICAL RULE**: The `Backend` project MUST remain **100% platform-independent**!

- ❌ **NEVER** add UI thread dispatching to Backend (`DispatcherQueue`, `MainThread`, etc.)
- ✅ **ALWAYS** handle platform-specific threading in **platform-specific ViewModels**

```csharp
// ❌ WRONG: Dispatching in Backend
public class JourneyManager
{
#if WINDOWS
    await DispatchToUIThreadAsync(...);  // ❌ BAD!
#endif
}

// ✅ CORRECT: Platform-specific ViewModel handles dispatching
namespace Moba.WinUI.ViewModels.Journey;

public class JourneyViewModel : SharedUI.ViewModel.JourneyViewModel
{
    private readonly DispatcherQueue _dispatcher;
    // Handle UI thread dispatching here ✅
}
```
...
```csharp
// ✅ CORRECT: Namespace matches project structure
...
⚠️ **Note:** MAUI.csproj defines `<RootNamespace>Moba.Smart</RootNamespace>` but code uses `Moba.MAUI` for consistency with other projects.
...
```csharp
// ✅ GOOD: Absolute namespace
using Moba.Backend.Model;
using Moba.SharedUI.Service;

// ❌ BAD: Relative namespace
using Backend.Model;  // Ambiguous!
```

### ✅ **Use Sub-Namespaces for Organization**
```csharp
// ✅ GOOD: Organized with sub-namespaces
namespace Moba.Backend.Manager;
...
## 📁 SharedUI Platform-specific subfolders
...
1. ✅ **Search Microsoft Learn** for official documentation
2. ✅ **Check for known issues** in .NET 10 / MAUI 10 / WinUI 3
3. ✅ **Propose alternatives** if technology is unstable
4. ✅ **Wait for explicit approval** before coding
...
- ✅ Production-ready (released May 20, 2025)
- ✅ Visual Studio 2026 (18.x) required
- ✅ Use C# 13 features where appropriate

### WinUI 3
- ✅ Windows App SDK
- ⚠️ **No XAML Designer** → Use XAML Hot Reload
- ✅ Use **x:Bind** over **Binding**

### MAUI 10 (Final Release)
- ✅ Production-ready for standard apps
- ✅ Android API 36 stable
- ⚠️ **Foreground Services still unstable** → Use native Android
- ⚠️ **Threading Rules CRITICAL!** → See [docs/THREADING.md](docs/THREADING.md)
...
{
    StatusText = "Received!";  // ❌ CRASH on Android!
}

// ✅ GOOD: Dispatched to Main Thread
private void OnUdpCallback(Data data)
{
#if ANDROID || IOS || MACCATALYST || WINDOWS
    MainThread.BeginInvokeOnMainThread(() =>
    {
        StatusText = "Received!";  // ✅ Safe
    });
#endif
}
```

📄 **Full Guidelines**: See [docs/THREADING.md](docs/THREADING.md)
...
### Key Rules:
- ✅ Use `async` for I/O-bound work (network, file, database)
- ✅ Suffix methods with `Async`
- ✅ Use `ConfigureAwait(false)` in Backend (not in ViewModels)
- ❌ **NEVER** use `.Result` or `.Wait()` (causes deadlocks)
- ❌ **NEVER** use `async void` (except event handlers)

📄 **Full Guidelines**: See [docs/ASYNC-PATTERNS.md](docs/ASYNC-PATTERNS.md)
...
📄 **Full Guidelines**: See [docs/UX-GUIDELINES.md](docs/UX-GUIDELINES.md)
...
- ❌ No loading indicators → ✅ Show ProgressRing for >1s operations
- ❌ Cryptic errors → ✅ Explain problem + suggest solution
- ❌ Fixed width layouts → ✅ Responsive with MaxWidth
- ❌ No tooltips → ✅ Tooltip on every button
- ❌ No confirmation → ✅ ContentDialog for destructive actions
...
⚠️ **CRITICAL RULE**: **NEVER** use `new` for services, ViewModels, or application state!
...
**Why Singleton?**
- ✅ There is only **one active Solution** per application instance
- ✅ All ViewModels share the **same instance** (shared state)
- ✅ Changes are **immediately visible everywhere**
- ✅ Represents the **central application state**
...
- ❌ **Scoped**: WinUI/MAUI don't have request scopes (Blazor concept)
- ❌ **Transient**: Would create multiple Solution instances → data loss!
- ✅ **Singleton**: Semantically correct for application-level state
...
```csharp
// ✅ CORRECT: Factory creates platform-specific ViewModel
...
1. ✅ **Register in ALL platforms** (WinUI, MAUI, Blazor)
2. ✅ **Choose correct lifetime**:
   - Singleton: Application-wide state, expensive to create
   - Scoped: Per-request (Blazor only)
   - Transient: Stateless, cheap to create
3. ✅ **Add to DI tests** (`Test/WinUI/WinUiDiTests.cs`, `Test/WebApp/WebAppDiTests.cs`)
4. ✅ **Update unit test mocks** in `Test/TestBase/ViewModelTestBase.cs`

📄 **Full DI Guidelines**: See [docs/DI-INSTRUCTIONS.md](docs/DI-INSTRUCTIONS.md)
