...
- Components: MOBAflow (Windows desktop), MOBAsmart (Android), MOBApi (REST API), Backend, TrackPlan, SharedUI, and the common/domain libraries shipped in the repo.
...
- Keep secrets out of source control (tracked `MOBAflow/appsettings*.json` files must stay secret-free; use User Secrets or environment variables for credentials).
