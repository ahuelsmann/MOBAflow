...
> **Status:** ✅ FUNKTIONSFAEHIG - App startet erfolgreich
...
|-----------|-------|--------|
| `INavigationService` | Zentrale Navigation mit History | ✅ Implementiert |
| `IPageFactory` | DI-basierte Page-Erstellung | ✅ Implementiert |
| `ILifecycleHost` | App-weite Lifecycle-Events | ✅ Implementiert |
| `IShellService` | Shell-Regions (Main, Overlay, Sidebar, Footer) | ✅ Implementiert |
| `ShellRegion` (enum) | Regionen-Definition | ✅ Implementiert |
| `PageDescriptor` (record) | Page-Metadaten | ✅ Implementiert |

**WinUI Implementierungen (WinUI/Service/):**

| Service | Beschreibung | Status |
|---------|--------------|--------|
| `NavigationService` | Erweitert um INavigationService, Navigation History | ✅ Erweitert |
| `PageFactory` | DI-basierte Page-Erstellung | ✅ Neu |
| `LifecycleHost` | WinUI Lifecycle-Integration | ✅ Neu |
| `ShellService` | Shell-Region-Management | ✅ Neu |
...
### Terminal Usage - RESTRICTION LIFTED ✅ (2026-01-14)

**Status:** Terminal commands are now ALLOWED again.

**What Changed:**
- Previous restriction (`no-terminal.instructions.md`) has been **lifted**
- Copilot can now use PowerShell commands for:
  - File operations
  - Git commands
  - Build/test automation
  - Diagnostics and verification
- **Rule:** Terminal commands MUST be valid PowerShell 7 syntax (Windows platform)
...
### Terminal Instructions Updated ✅ (2026-01-14)

**Status:** `terminal.instructions.md` has been updated with hard rules for PowerShell usage.

**What Changed:**
- File: `.github/instructions/terminal.instructions.md` (ACTIVE - not deleted!)
- Contains: PowerShell 7 command chaining rules, syntax requirements, error handling
- Replaced: `no-terminal.instructions.md` (deleted)
- Copilot-Instructions: Updated to reflect 4 active files (not 3)

**Key Rules in terminal.instructions.md:**
- ✅ NEVER use `&&` (Bash/CMD) - use `;` instead
- ✅ Command chaining with PowerShell syntax
- ✅ Exit code handling via `$LASTEXITCODE`
- ✅ Windows paths with proper quoting
- ✅ PowerShell cmdlets (Get-ChildItem, Select-String, etc.) instead of Unix tools

**Verification:** Terminal tests passed successfully (94/94 tests, git operations work)

---

## INSTRUCTION FILES STATUS (Dynamischer Index)

> **Authoritative source for all instruction documentation.**  
> **See:** [.github/instructions/copilot-instructions.md](./copilot-instructions.md) for stable references.

### Active Instruction Files (4 Total)

| File | Status | Purpose | Last Updated |
|------|--------|---------|--------------|
| **copilot-instructions.md** | ✅ Active | Central index (minimal, stable references) | 2026-01-14 |
| **self-explanatory-code-commenting.instructions.md** | ✅ Active | Code commenting guidelines (WHY not WHAT) | Current |
| **terminal.instructions.md** | ✅ Active | PowerShell 7 hard rules for terminal usage | 2026-01-14 (Updated) |
| **.copilot-todos.md** | ✅ Active | This file - Dynamic knowledge bridge | 2026-01-14 |

### YAML Frontmatter Standard (All Files MUST Follow)

```yaml
---
description: 'One-sentence description of instruction scope and purpose'
applyTo: '** (applies everywhere) or specific glob pattern'
---
...
**Example from self-explanatory-code-commenting.instructions.md:**
```yaml
---
description: 'Guidelines for GitHub Copilot to write self-explanatory code with minimal comments'
applyTo: '**'
---
```

### Removed Files (Intentionally Deleted - Do NOT Re-add without Substantial Content)

These 17 files were intentionally deleted as placeholder/false content:
- Track Planning: geometry.md, topology.md, snapping.md, rendering.md, editor-behavior.md
- Architecture: backend.instructions.md, collections.instructions.md, di-pattern-consistency.instructions.md, dotnet-framework.instructions.md
- UI: winui.instructions.md, maui.instructions.md, blazor.instructions.md, xaml-page-registration.instructions.md
- Testing: test.instructions.md, hasunsavedchanges-patterns.instructions.md
- DevOps: github-actions-ci-cd-best-practices.instructions.md, powershell.instructions.md
- Copilot: instructions.instructions.md, prompt.instructions.md, **no-terminal.instructions.md** (replaced by `terminal.instructions.md`)

**Note:** `no-terminal.instructions.md` was the old restriction file. It has been replaced by `terminal.instructions.md` which now contains hard rules for PowerShell 7 usage and IS an active file.

### Future Instruction Files (Planned, Not Created Yet)

| File | Status | Planned For | Notes |
|------|--------|------------|-------|
| [More TBD] | ⏳ Future | As needed | Document in this section before creating file |

---

## NEUE INITIATIVE: 0 ReSharper Warnungen Policy (2026-01-14)

> **Ziel:** Codequalitaet und Maintainability durch systematisches Aufloesen aller Compiler-Warnungen erhoehen.

### Motivation & Scope

**Status:** Initial Analysis Done (siehe Analyse unten)
**Prioritaet:** Stufe 2 (nach Security/Copyright, vor Features)
**Komplexitaet:** Mittel (viele kleine Fixes + wenige Architektur-Entscheidungen)

### Erkenntnisse der Analyse (2026-01-14)

**Warnungs-Kategorien identifiziert:**

1. **Redundante Qualifiers** (z.B. `Microsoft.UI.Colors.Gray` -> `Colors.Gray`)
   - Status: **FIXABLE** - Bereits teilweise behoben
   - Aufwand: Niedrig (nur Using-Direktiven aufraeumen)
   - Beispiele: CanvasRenderer.cs, TrackPlanEditorPage.xaml.cs

2. **Unbenutzte lokale Variablen** (z.B. `out var pos` statt `out _`)
   - Status: **FIXABLE** - Bereits teilweise behoben
   - Aufwand: Niedrig (discard operator einsetzen)
   - Beispiel: CanvasRenderer.cs Zeile 40

3. **Unbenutzte Field Assignments** (z.B. `_panStartH`, `_panStartV`)
   - Status: **FIXABLE** - Bereits behoben
   - Aufwand: Niedrig (entfernen oder Code-Path implementieren)
   - Beispiel: TrackPlanEditorPage.xaml.cs Zeile 40-41

4. **Unbenutzte Parameter in Events/Interfaces**
   - Status: **REVIEW REQUIRED** - Nicht einfach loeschen (externe Vertraege)
   - Aufwand: Mittel (Analyse erforderlich)
   - Beispiele: SignalBoxPageBase.cs (s, e Parameter)
   
5. **Nullable Reference Type Probleme**
   - Status: **REVIEW REQUIRED** - Bedingte Accesses auf non-null Types
   - Aufwand: Mittel (semantische Analyse)
   - Beispiele: StatisticsPluginViewModel.cs, WinUI/View/HelpPage.cs

6. **Partial Classes mit nur einer Part**
   - Status: **ARCHITECTURAL** - Kann konsolidiert werden
   - Aufwand: Hoch (XAML-Code-Behind Koordination)
   - Beispiele: SignalBoxPlanViewModel.cs (3 Parts)

7. **Ungenutzte Private Members**
   - Status: **DECISION REQUIRED** - Vor Loeschung pruefen
   - Aufwand: Mittel (Code-Review + Intention klaeoren)
   - Beispiele: CmdPlugin (unused colors), SpeedometerControl (unused Field)

### Implementierungs-Strategie (3 Phasen)

#### **PHASE 1: Quick Wins (Sofort - Diese Woche)**
**Ziel:** Maximale Wirkung bei minimalem Risiko
**Warnungen reduzieren:** ~50 -> ~25

| Aufgabe | Dateien | Aufwand | ReSharper IDs |
|---------|---------|---------|----------------|
| 1. Redundante Qualifiers entfernen | CanvasRenderer.cs, TrackPlanEditorPage.xaml.cs, TrackConnectionService.cs, etc. (25+ Dateien) | 2-3h | CS0162, CS0219 |
| 2. Discard-Operator (_) einsetzen | Alle "out var pos" -> "out _" Faelle | 1h | CS0168 |
| 3. Ungenutzte lokale Variablen loeschen | CanvasRenderer (pos), TrackPlanEditorPage (json), SignalBoxPage (currentTemplate, localPortId) | 1h | CS0168 |
| 4. Ungenutzte Field Assignments entfernen | TrackPlanEditorPage (_panStartH, _panStartV) + panning code cleanup | 1h | CS0169 |
| **Phase 1 Total** | | **5-6h** | **~50 Warnungen** |

**Bereits abgeschlossen (2026-01-14):**
- ✅ CanvasRenderer.cs Zeile 40 (discard)
- ✅ CanvasRenderer.cs Zeile 86 (Colors.Gray)
- ✅ TrackPlanEditorPage.xaml.cs _panStartH/_panStartV entfernt + panning cleanup
- ✅ TrackPlanEditorPage.xaml.cs NumberStyles.HexNumber qualifier entfernt
- ✅ TrackPlanEditorPage.xaml.cs Colors.* qualifiers entfernt

#### **PHASE 2: Code-Review erforderlich (Naechste 2 Wochen)**
**Ziel:** Sichere Entfernung von Privacy-Violations und Nullable-Issues
**Warnungen reduzieren:** ~25 -> ~10

| Aufgabe | Dateien | Aufwand | Notes |
|---------|---------|---------|-------|
| 1. Ungenutzte private Fields | CmdTransactionContentProvider (CmdGold, CmdDarkBlue, etc.), WinUI Controls | 3-4h | Pruefen ob dekorativer Zweck (Designzeit-Referenzen) |
| 2. Ungenutzte private Methods | CmdTransactionContentProvider.CommandHistoryListView_ItemClick | 2h | Vor Loeschung: Event-Handler-Nutzung pruefen |
| 3. Nullable Reference Type Issues | StatisticsPluginViewModel (?. auf non-null), WinUI/View/HelpPage (expression always true) | 4-5h | Semantische Analyse erforderlich, ggf. Tests anpassen |
| 4. Unbenutzte Parameter in Events | Viele XAML Event-Handler (s, e nie genutzt) | 3h | **ENTSCHEIDUNG:** Parameter umbenennen zu `_` ODER discard-only parameter verwenden? |
| **Phase 2 Total** | | **12-14h** | **~15 Warnungen** |

**Status:** Pending

#### **PHASE 3: Architektur-Entscheidungen (Groessere Refactorings)**
**Ziel:** Strukturelle Verbesserungen fuer zukuenftige Wartbarkeit
**Warnungen reduzieren:** ~10 -> ~0

| Aufgabe | Kategorie | Aufwand | Abhaengigkeiten |
|---------|-----------|---------|-----------------|
| 1. Partial Classes konsolidieren | SignalBoxPlanViewModel (3 Parts) | 3-4h | XAML-Code-Behind refactoring |
| 2. Unused Parameter Konvention festlegen | Alle Event-Handler | 4-5h | Team-Decision: Discard-Parameter erlaubt? |
| 3. Panning-Feature: Implementieren oder Entfernen | TrackPlanEditorPage | 5-8h | Business-Decision: Wird Feature benoetigt? |
| **Phase 3 Total** | | **12-17h** | **DECISION POINTS** |

**Status:** On Hold (Strategic Decisions erforderlich)

### ReSharper Konfiguration (.editorconfig)

**Empfohlen fuer MOBAflow:**

```editorconfig
# .editorconfig - ReSharper 0-Warnings Policy

# SEVERITY SETTINGS
[*.cs]
csharp_naming_rule.public_methods_PascalCase.severity = error
csharp_naming_rule.private_fields_camelCase.severity = warning

# INSPECTION SEVERITY
# Unused Members
dotnet_diagnostic.CS0169.severity = warning    # Unused field
dotnet_diagnostic.CS0168.severity = warning    # Unused variable
dotnet_diagnostic.CS0162.severity = warning    # Unreachable code

# Nullable Reference Types
dotnet_diagnostic.CS8632.severity = warning    # Conditional access on non-null
dotnet_diagnostic.CS8629.severity = warning    # Nullable reference types

# Redundant Code
dotnet_diagnostic.CS0219.severity = suggestion # Assigned but never used
dotnet_diagnostic.IDE0005.severity = warning   # Remove unused imports

# Event Handler Parameters - DECISION PENDING
# Momentan: Parameter nicht als unused markiert (zu laut)
# Optionen:
# A) Alle Handler-Parameter umbenennen zu _, __ (mit DisabledWarningAttribute)
# B) Spezielle Konvention: (object? _, EventArgs _) akzeptiert
# C) Keine Warnung fuer Event-Handler (Filter-Config)
```

**Zu implementieren:** `.editorconfig` in Workspace-Root mit Severity-Einstellungen
  ...
  analyze:
    ...
    steps:
      ...
      - name: ReSharper Code Inspection
        ...
        with:
          solution: MOBAflow.sln
          fail-level: warning
```

**Status:** NICHT IMPLEMENTIERT (abhaengig von Phase 1-2 Completion)

### Team-Entscheidungen erforderlich (TODOS)

| Entscheidung | Optionen | Empfehlung | Status |
|--------------|----------|-----------|--------|
| **Event-Handler Parameter** | A) Umbenennen zu `_`, B) Discard-only params, C) Filter aus ReSharper | B) Best Practice in C# 9+ | ⏳ PENDING |
| **Private Field Retention** | A) Alle ungenutzt loeschen, B) dekorative Fields behalten | B) Nur explizit genutzte behalten | ⏳ PENDING |
| **Nullable Strictness** | A) Strikt (alle ! entfernen), B) Pragma suppress, C) `#nullable disable` | A) Strikt progressiv (ein Projekt nach dem anderen) | ⏳ PENDING |
| **Panning Feature** | A) Implementieren, B) Entfernen | B) Entfernen (bereits ohne UI-Trigger) | ✅ DONE (entfernt 2026-01-14) |

### Fortschritt-Tracking

```
Phase 1 (Quick Wins):       ████████████░░░░░░░░░░░░░░░░░░░ 40% (Warnungen: 50 -> 45?)
Phase 2 (Code Review):      ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  0% (Pending)
Phase 3 (Architecture):     ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  0% (Pending)

GESAMT:                     ~13% abgeschlossen
ZIEL-FINISH:                ca. 3-4 Wochen (mit parallelen Features)
```

### Dokumentation

**Neu erstellt:** keine (nutzt bestehende WinUI/Dotnet Guidelines)

**Zu aktualisieren:**
- `.editorconfig` mit ReSharper-Einstellungen
- `.github/instructions/dotnet-framework.instructions.md` - "0 Warnings Policy" Abschnitt
- `CONTRIBUTING.md` - "Code Quality Standards" Abschnitt
...
| Done | **Plugin Navigation Fix** | Plugins werden jetzt VOR Help/Info/Settings in der NavigationView einfuegt. SamplePlugin Build-Artefakt geloescht. |
| Done | **Panning Fix** | TrackPlanEditorPage: Rechtsklick-Panning mit Button-State-Verifikation und Exception-Handling. |
| Done | **Snap-to-Connect Root Fix** | TopologySolver verwendet Target-Segment als Root beim Verbinden (behält Position bei). |
...
| Status | Aufgabe | Details |
|--------|---------|---------|
| Done | **PortChecker Utility erstellt** | `WinUI\Utilities\PortChecker.cs` fuer Port-Verfügbarkeitspruefung |
...
| Done | **CODE_OF_CONDUCT.md** | Erstellt 2026-01-14 |
| Done | **SECURITY.md** | Erstellt 2026-01-14 |
...
| Pending | **Safety/Assist (Bremskurven)** | Brems- und Beschleunigungsprofile pro Lok konfigurierbar (soft/hard), optional Not-Halt-Countdown und Brake-Assists; Profile im Settings/TrainControl hinterlegbar, ggf. eigene Assist-Overlay-Page. |
...
- **Dokumentation:** 8/9 (89%)
- **Repository-Konfiguration:** 2/7 (29%)
- **Code-Bereinigung:** 3/5 (60%)
- **Tests:** 1/3 (33%)

**Gesamtfortschritt:** ca. 56% abgeschlossen
...
2. **Dark Mode fuer Blazor** - Groesseres Feature, seperate Session
3. **Real-time Charts fuer Blazor** - Groesseres Feature, seperate Session
4. **Fuehrerstand-Page (Cab Simulator)** - ETCS/PZB-Anzeigen, Fahrschalter/Bremscontroller/Sifa, nutzt TrainControl-Logik
5. **Safety/Assist (Bremskurven)** - Profile pro Lok, sanfte Rampen, optional Not-Halt-Countdown/Overlay

### Dokumentation
1. CHANGELOG.md erstellen
2. GitHub Actions CI/CD Workflow erstellen
