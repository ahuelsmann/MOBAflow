...
public partial class AppTabHostPage
{
    ...
    private void AttachTabSwipeGesture()
    {
        var panGesture = new PanGestureRecognizer();
        panGesture.PanUpdated += OnTabSwipePanUpdated;
        TabContentGrid.GestureRecognizers.Add(panGesture);
    }

    private void OnTabSwipePanUpdated(object? sender, PanUpdatedEventArgs e)
    {
        ...
        if (Math.Abs(e.TotalX) < SwipeThreshold || Math.Abs(e.TotalX) <= Math.Abs(e.TotalY))
        {
            return;
        }

        ShowTab(e.TotalX < 0
            ? Math.Min(_activeTabIndex + 1, TabCount - 1)
            : Math.Max(_activeTabIndex - 1, 0));
    }
