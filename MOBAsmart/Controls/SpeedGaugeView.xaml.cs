...

{
...

    {
        ...
        _drawable.IsLightBackground = ResolveIsLightTheme(_drawable.SurfaceColor, _drawable.TextPrimary);



        // Stale resource dictionary can still expose light-on-light tokens; force readable ink.

        if (_drawable.IsLightBackground && IsLightColor(_drawable.TextPrimary))

        {

            _drawable.TextPrimary = Color.FromArgb("#212121");

            _drawable.TextSecondary = Color.FromArgb("#616161");
    ...
    private static bool ResolveIsLightTheme(Color surface, Color textPrimary)

    {

        var effectiveTheme = GetEffectiveAppTheme();
        ...
        return IsLightColor(surface) || !IsLightColor(textPrimary);
    ...
    private static bool IsLightColor(Color color) =>
        ...
        (0.2126 * color.Red) + (0.7152 * color.Green) + (0.0722 * color.Blue) >= 0.55;
