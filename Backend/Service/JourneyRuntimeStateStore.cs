...
public sealed record JourneyRuntimeCheckpoint(Guid? CurrentStationId, Guid JourneyRunId);
...
public sealed class FileJourneyRuntimeStateStore : IJourneyRuntimeStateStore
{
    ...
    public void Save(Guid projectId, JourneySessionState state)
    {
        ...
        {
            var data = Read();
            data[Key(projectId, state.JourneyId)] = new(state.CurrentStationId, state.RunId);
            Write(data);
        }
    }
