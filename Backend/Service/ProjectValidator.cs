...
public class ProjectValidator : IProjectValidator
{
    ...
    private static void ValidateJourneys(Project project, string projectName, ProjectValidationResult result)
    {
        ...
        else
        {
            ...
            {
                ValidateEventPlan(project, journey, projectName, result);
            }
        }
    }

    private static void ValidateEventPlan(Project project, Journey journey, string projectName, ProjectValidationResult result)
    {
        var events = journey.EventPlan.Events;
        for (var index = 0; index < events.Count; index++)
        {
            var journeyEvent = events[index];
            var prefix = $"[{projectName}/{journey.Name}/Event {index + 1}]";
            if (journeyEvent.InPort is < 1 or > 512)
                result.AddError($"{prefix} InPort must be between 1 and 512.");
            if (journeyEvent.Count < 1)
                result.AddError($"{prefix} Count must be at least 1.");
            if (journeyEvent.WorkflowId is Guid workflowId && project.Workflows.All(workflow => workflow.Id != workflowId))
