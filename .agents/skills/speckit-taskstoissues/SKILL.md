...
1. Resolve the destination from the active worktree. Run the repository helper below and use its JSON repository/url fields for every issue operation. Stop on failure; do not fall back to origin or guess the owner. A GitHub tracking remote takes precedence; otherwise exactly one GitHub remote is required. Run:

```bash
pwsh -NoProfile -File scripts/Resolve-GitHubRepository.ps1
```
