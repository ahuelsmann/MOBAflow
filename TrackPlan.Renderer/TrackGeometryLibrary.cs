...
public class TrackGeometryLibrary
{
    ...
    private void InitializeTemplates()
    {
        ...
        // End: x = 180.00, y ≈ 48.23
        // SVG: A rx ry xAxisRotation largeArcFlag sweepFlag x y
        ...
        // R9 - Radius 907.97mm, 15°
        // End: x ≈ 235.00, y ≈ 30.93
        ...
        // Straight: 0° → 180° (L)
        // Diverging: R9, 15° right (A)
        _templates["WR"] = new TrackGeometry
        {
            ArticleCode = "WR",
            Endpoints = [
                new TrackPoint(0, 0),          // [0] Entry
                new TrackPoint(230.93, 0),     // [1] Straight exit
                new TrackPoint(235.00, 30.93)  // [2] Diverging exit (R9, 15°)
            ],
            EndpointHeadingsDeg = [0, 180, 195],
            PathData =
                "M 0,0 L 230.93,0 " +
                "M 0,0 A 907.97,907.97 0 0 1 235.00,30.93"
        };

        // WL - Left Regular Turnout (Linksweiche) - Piko 55421
        // Diverging: R9, 15° left → sweepFlag = 0
        _templates["WL"] = new TrackGeometry
        {
            ...
            EndpointHeadingsDeg = [0, 180, 165],
            PathData =
                "M 0,0 L 230.93,0 " +
                "M 0,0 A 907.97,907.97 0 0 0 235.00,-30.93"
        };
        ...
        _templates["W3"] = new TrackGeometry
        {
            ...
            Endpoints = [
                new TrackPoint(0, 0),           // [0] Entry
                new TrackPoint(230.93, 0),      // [1] Straight exit
                new TrackPoint(235.00, 30.93),  // [2] Diverging right
                new TrackPoint(235.00, -30.93)  // [3] Diverging left
            ],
        ...
        _templates["WY"] = new TrackGeometry
        {
            ...
            Endpoints = [
                new TrackPoint(0, 0),           // [0] Entry
                new TrackPoint(235.00, 30.93),  // [1] Right branch
                new TrackPoint(235.00, -30.93)  // [2] Left branch
            ],
            EndpointHeadingsDeg = [0, 195, 165],
            PathData =
                "M 0,0 A 907.97,907.97 0 0 1 235.00,30.93 " +
                "M 0,0 A 907.97,907.97 0 0 0 235.00,-30.93"
        };
        ...
        _templates["BWR"] = new TrackGeometry
        {
            ...
            PathData =
                "M 0,0 A 421.88,421.88 0 0 1 210.94,56.52 " +
                "M 0,0 A 483.75,483.75 0 0 1 241.88,64.80"
        };

        // BWL - Curved Switch Left R2→R3
        ...
        // BWR-R3 - Curved Switch Right R3→R4
        _templates["BWR-R3"] = new TrackGeometry
        {
            ...
            PathData =
                "M 0,0 A 483.75,483.75 0 0 1 241.88,64.80 " +
                "M 0,0 A 545.63,545.63 0 0 1 272.82,73.08"
        };

        // BWL-R3 - Curved Switch Left R3→R4
        ...
        _templates["K30"] = new TrackGeometry
        {
            ...
            Endpoints = [
                ...
                new TrackPoint(27.60, 15.47),   // [2] Entry B
                new TrackPoint(134.92, 15.47)   // [3] Exit B
            ],
            EndpointHeadingsDeg = [0, 180, 30, 210],
            PathData =
                "M 0,0 L 107.32,0 " +
                "M 27.60,15.47 L 134.92,15.47"
        };

        // === ANYRAIL IMPORT COMPATIBILITY ===
        // Legacy geometries for AnyRail imports (different radii)

        // Curve_545 - Radius 545mm, 30°
        _templates["Curve_545"] = new TrackGeometry
        {
            ArticleCode = "Curve_545",
            Endpoints = [new TrackPoint(0, 0), new TrackPoint(272.50, 73.05)],
            EndpointHeadingsDeg = [0, 30],
            PathData = "M 0,0 A 545,545 0 0 1 272.50,73.05"
        };

        // Curve_638 - Radius 638mm, 30°
        _templates["Curve_638"] = new TrackGeometry
        {
            ArticleCode = "Curve_638",
            Endpoints = [new TrackPoint(0, 0), new TrackPoint(319.00, 85.48)],
            EndpointHeadingsDeg = [0, 30],
            PathData = "M 0,0 A 638,638 0 0 1 319.00,85.48"
        };

        // Curve_732 - Radius 732mm, 30°
        _templates["Curve_732"] = new TrackGeometry
        {
            ArticleCode = "Curve_732",
            Endpoints = [new TrackPoint(0, 0), new TrackPoint(366.00, 98.10)],
            EndpointHeadingsDeg = [0, 30],
            PathData = "M 0,0 A 732,732 0 0 1 366.00,98.10"
        };

        // Curve_1374 - Radius 1374mm, 15°
    ...
    public void RegisterDynamicGeometry(
        string articleCode,
        List<TrackPoint> endpoints,
